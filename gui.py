...
frame_width, frame_height = 160, 160  # Reduced frame size for faster processing
process_interval = 3  # Process every 3rd frame
confidence_threshold = 0.4  # Confidence threshold for displaying predictions
...
while True:
    ...
    # Display the live video feed with prediction and confidence
    display_frame = cv2.resize(frame, (640, 480))  # Scale up for better display if needed
    cv2.putText(display_frame, f'Predicted: {predicted_label}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    cv2.putText(display_frame, f'Confidence: {confidence_display}', (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
